...
// 1. CORRECTED IMPORT: Get types from the central 'posts' store
import type { Poll, PollOption } from '@/stores/posts';
...
function getPercentage(option: PollOption): number {
  ...
  // Use 'vote_count' to match the updated interface
  return Math.round((option.vote_count / props.poll.total_votes) * 100);
}

// Replace the handleVote function in PollDisplay.vue with this one
...
async function handleVote(optionId: number) {
  ...
  isVoting.value = true;
  try {
    // This now sends the poll's ID and the selected option's ID,
    // which is exactly what your backend needs.
  ...
  <div class="p-4">
    ...
    <div class="space-y-3">
      <div v-for="(option, index) in poll.options" :key="option.id">
        <button @click="handleVote(option.id)" :disabled="isVoting"
          ...
          <!-- Progress Bar -->
          <!-- Progress Bar -->
          <div v-if="poll.total_votes > 0" class="absolute top-0 left-0 h-full bg-blue-100 transition-all duration-500"
            :style="{ width: getPercentage(option) + '%' }"></div>
          ...
          <div v-if="poll.total_votes > 0" class="relative flex items-center gap-3">
            ...
            <div class="w-5 h-5">
              <svg v-if="poll.user_vote === option.id" class="text-blue-600" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="text-xs text-gray-500 mt-3 text-right">{{ poll.total_votes }} vote<span
        v-if="poll.total_votes !== 1">s</span></div>
  </div>
</template>