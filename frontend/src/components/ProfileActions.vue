...
// Destructure all necessary state from the store
...
const handleFollow = () => {
    if (currentProfile.value) {
        ...
        if (relationshipStatus.value) {
            // If they were following us, we are now connected.
            if (relationshipStatus.value.follow_status === 'followed_by') {
...
const handleUnfollow = () => {
    if (currentProfile.value) {
        ...
        if (relationshipStatus.value) {
            relationshipStatus.value.follow_status = 'not_following';
            // If we were connected, unfollowing breaks the connection
            if (relationshipStatus.value.connection_status === 'connected') {
                relationshipStatus.value.connection_status = 'not_connected';
            }
        }
    }
}
...
// TODO: Implement handleAccept and handleReject actions in the store and here.
</script>
...
<template>
    <div v-if="relationshipStatus" class="mt-4 flex items-center justify-center space-x-3">
        ...
        <div class="flex-1">
            ...
            <button v-else-if="relationshipStatus.connection_status === 'connected'" disabled
