...
export const useProfileStore = defineStore('profile', () => {
  ...
  function handlePostDeletedSignal(postId: number) {
    ...
    for (const username in postIdsByUsername.value) {
      postIdsByUsername.value[username] = postIdsByUsername.value[username].filter(id => id !== postId);
    }
  }

  async function fetchProfile(username: string) { if (profilesByUsername.value[username]) { currentProfile.value = profilesByUsername.value[username]; isFollowing.value = currentProfile.value.is_followed_by_request_user; return; } isLoadingProfile.value = true; errorProfile.value = null; try { const response = await axiosInstance.get<UserProfile>(`/profiles/${username}/`); const profile = response.data; profilesByUsername.value[username] = profile; currentProfile.value = profile; isFollowing.value = profile.is_followed_by_request_user; } catch (err: any) { errorProfile.value = err.response?.data?.detail || `Profile not found for user "${username}".`; } finally { isLoadingProfile.value = false; } }
  ...
  function addPostToProfileFeed(post: Post) { const username = post.author.username; if (postIdsByUsername.value[username]) { postIdsByUsername.value[username].unshift(post.id); } }
  
  async function updateProfilePicture(username: string, pictureFile: File) { const formData = new FormData(); formData.append('picture', pictureFile); try { const response = await axiosInstance.patch<UserProfile>(`/profiles/${username}/`, formData); const updatedProfile = response.data; profilesByUsername.value[username] = updatedProfile; if (currentProfile.value?.user.username === username) { currentProfile.value = updatedProfile; } if (authStore.currentUser?.username === username) { authStore.updateCurrentUserPicture(updatedProfile.picture!); } const authorUpdates: Partial<PostAuthor> = { picture: updatedProfile.picture }; const allPosts = Object.values(postsStore.posts); const postsToUpdate = allPosts.filter(p => p.author.id === updatedProfile.user.id); const partialPostsToUpdate = postsToUpdate.map(p => ({ id: p.id, author: { ...p.author, ...authorUpdates } })); postsStore.addOrUpdatePosts(partialPostsToUpdate); return updatedProfile; } catch (err: any) { throw new Error(err.response?.data?.picture?.join(' ') || err.response?.data?.detail || 'Failed to update picture.'); } }

  async function followUser(usernameToFollow: string) { if (isLoadingFollow.value) return; isLoadingFollow.value = true; try { await axiosInstance.post(`/users/${usernameToFollow}/follow/`); if (currentProfile.value) currentProfile.value.is_followed_by_request_user = true; isFollowing.value = true; } finally { isLoadingFollow.value = false; } }
