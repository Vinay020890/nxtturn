...
describe('Post Interaction Flow', () => {
  ...
  it('should allow a user to create, comment on, like, and delete their own post', () => {
    ...
    cy.get('[data-cy="post-container"]')
      .contains(uniquePostText)
      .parents('[data-cy="post-container"]')
      .as('thePost')
    ...
    // --- 3. LIKE POST ---
    // Initially, there are 0 likes, so the summary row should not even exist
    cy.get('@thePost').find('[data-cy="reaction-summary"]').should('not.exist')

    // Click the like button
    cy.get('@thePost').find('[data-cy="like-button"]').click()

    // Now, the summary row should appear and contain the word "You"
    cy.get('@thePost')
      .find('[data-cy="reaction-summary"]')
      .should('be.visible')
