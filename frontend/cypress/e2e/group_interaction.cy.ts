...
describe('Group Creation and Interaction', () => {
  ...
  it('a creator can deny a join request for a private group', () => {
    ...
    cy.testSetup('create_user', { email: `${creatorUsername}@test.com`, username: creatorUsername, password: 'password' })
      .then(() => cy.testSetup('create_user', { email: `${deniedUsername}@test.com`, username: deniedUsername, password: 'password' }))
      .then(() => {
        cy.login(creatorUsername, 'password');
        cy.visit('/groups');
        cy.get('[data-cy="create-group-button"]').click();
        ...
        cy.get('@groupPageUrl').then(url => cy.visit(url as unknown as string));
        cy.get('[data-cy="group-membership-button"]').should('contain.text', 'Request to Join').click();
        ...
        cy.intercept('PATCH', `**/api/groups/${groupSlug}/requests/**/`).as('manageRequest');
        cy.contains('div.p-4', deniedUsername)
          .within(() => {
  ...
  it('a post in a private group does not appear on a non-member follower\'s home feed', () => {
    ...
    cy.testSetup('create_user', { email: `${creatorUsername}@test.com`, username: creatorUsername, password: 'password' })
      ...
      .then(() => {
        ...
        cy.get('[data-cy="create-group-button"]').click();
        cy.get('[data-cy="group-name-input"]').type(groupName);
        cy.get('[data-cy="group-description-input"]').type('A group for testing private feed visibility.');
        cy.get('input[value="private"]').check();
        cy.get('[data-cy="create-group-submit-button"]').click();
        cy.url().should('include', `/groups/${groupSlug}`);
  ...
  it('a creator can deny and block a user, and later unblock them', () => {
    ...
    // Part 1: Setup
    cy.testSetup('create_user', { email: `${creatorUsername}@test.com`, username: creatorUsername, password: 'password' })
      .then(() => cy.testSetup('create_user', { email: `${blockedUsername}@test.com`, username: blockedUsername, password: 'password' }))
      .then(() => {
        cy.login(creatorUsername, 'password');
        cy.visit('/groups');
        cy.get('[data-cy="create-group-button"]').click();
        cy.get('[data-cy="group-name-input"]').type(groupName);
        cy.get('[data-cy="group-description-input"]').type('A group for testing the block/unblock flow.');
        cy.get('input[value="private"]').check();
        cy.get('[data-cy="create-group-submit-button"]').click();
        cy.url().should('include', `/groups/${groupSlug}`).as('groupPageUrl');
        cy.logout();
        ...
        cy.get('[data-cy="group-membership-button"]').should('be.disabled').and('contain.text', 'Blocked');
        cy.logout();
      }).then(() => {
        ...
        cy.contains('div.p-4', blockedUsername).within(() => cy.contains('button', 'Unblock').click());
        cy.wait('@unblockUser');
        cy.logout();
      }).then(() => {
        ...
        cy.login(blockedUsername, 'password');
        cy.get('@groupPageUrl').then(url => cy.visit(url as unknown as string));
        cy.get('[data-cy="group-membership-button"]').should('not.be.disabled').and('contain.text', 'Request to Join');
      });
  });
});