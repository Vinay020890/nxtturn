...
# tests/community/test_registration_api.py
...
def test_registration_fails_with_duplicate_email(api_client):
    ...
    Updated to match the professional 'NxtTurn' error format.
    """
    url = "/api/auth/registration/"
    user1_data = {
    ...
    api_client.post(url, user1_data)
    ...
    # Assertions updated for our new "Better Error"
    assert response2.status_code == status.HTTP_400_BAD_REQUEST
    assert "non_field_errors" in response2.data
    assert (
        "An account with this email already exists"
        in response2.data["non_field_errors"][0]
    )
